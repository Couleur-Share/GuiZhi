...
- **论坛目前支持 V2EX 与 NGA**；其它站点暂无专用连接器，会退回通用网页抓取（效果通常较差）。NGA 公开帖无需登录；需登录版块仍采不到。NGA 不镜像整帖水楼（讨论区只留楼主回复，并尽量附带被回复楼摘要）；不做泥潭像素级复刻（无侧栏/徽章/表格排版），折叠与强调色等常见 BBCode 会转成可读结构。附件图只处理主楼与楼主回复，上限 80 张，超出保留外链。详情页重新生成总结时，NGA 条目只能基于已入库的楼主回复。讨论里点「回复 @某人」时，被回复楼若未入库（NGA 常见）或旧条目无楼层号且作者不唯一，只能提示无法定位，不会为此重采旧帖
...
- **需要登录才能看的内容**抓不到（平台 cookies 采集未实现，仍延后）
