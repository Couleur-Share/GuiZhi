...
- 平台、知识库与标签支持组合筛选；改变筛选会清空原来的跨页选择。

## AI 与媒体

- **正文配图**目前适配 OpenAI 文生图与 Gemini；Anthropic 无对应 API
- **嵌入**按 OpenAI 请求格式实现（Gemini 可走兼容层；Anthropic 无 embeddings）
- **语义检索**在 50,000 分块、256 MiB 向量或热态中位耗时超过 500ms 时尝试独立 worker 的 HNSW。当前依赖在本次 Windows Electron 验收中不兼容，会明确回退精确余弦检索；不把回退成功当作加速性能通过。问答侧栏显示分块数、耗时与回退原因。
- 用量面板只数调用次数与 token，不折算费用；生图与转写接口本就不回报 token 时会显示「N 次 · 0」
- 设置页的文生图 / 转写连接测试会真实计费

## 历史证据与全文编译

- 旧问答没有保存的片段无法事后重建；界面明确标注历史降级。清除来源证据不改写回答文本。
- 历史 Wiki 全文升级需主动选批；需要模型调用，不在升级时后台自动运行。分块完成不等于整篇完成，人工页面需要审阅更新建议。
- 来源更新版本保留最近 20 条的浏览入口；数据库与完整备份保留版本记录。
- 具体操作和本批尚未发布的边界见 [重度使用改进](./heavy-user-improvements.md)。
