...
export function researchEligibility(candidate: ResearchCandidate, run?: Pick<ResearchRun, "rangeFrom" | "rangeTo" | "timeScope">, plan?: ResearchPlan): ResearchEligibility {
  ...
  if (candidate.publishedAt != null && run && run.timeScope !== "all" && (candidate.publishedAt < run.rangeFrom || candidate.publishedAt > run.rangeTo)) return "out_of_window";
  return "recent";
}

export function selectResearchEvidence(candidates: readonly ResearchCandidate[], run?: Pick<ResearchRun, "rangeFrom" | "rangeTo" | "timeScope">, plan?: ResearchPlan, hashes = new Map<string, string>(), max: number = RESEARCH_POLICY.maxEvidence): ResearchCandidate[] {
  const sorted = [...candidates].filter((c) => ["recent", "undated"].includes(researchEligibility(c, run, plan)))
    .sort((a, b) => (run?.timeScope === "all" ? 0 : Number(researchEligibility(a, run, plan) === "undated") - Number(researchEligibility(b, run, plan) === "undated")) || b.overallScore - a.overallScore || a.id.localeCompare(b.id));
  ...
  const add = (candidate: ResearchCandidate) => {
    ...
    if (result.length >= max || groups.has(group) || (authors.get(author) ?? 0) >= RESEARCH_POLICY.maxAuthor || (candidate.source !== "web" && (sources.get(candidate.source) ?? 0) >= RESEARCH_POLICY.maxSource) || (run?.timeScope !== "all" && unknown && undated >= RESEARCH_POLICY.maxUndated)) return false;
  ...
  const sourceOrder = ["xiaohongshu", "douyin", "bilibili", "web"] as const;
