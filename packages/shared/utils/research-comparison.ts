...
export function compareResearch(current: ResearchRunDetail, baseline?: ResearchRunDetail | null): ResearchComparison {
  ...
  const key = (c: ResearchRunDetail["candidates"][number]) => `${c.source}:${c.externalId || c.normalizedUrl}`;
  ...
  for (const [identity, candidate] of now) {
    ...
    if (previous.dateConfidence !== "low" && previous.publishedAt != null && previous.publishedAt < current.run.rangeFrom) { result.changes.push({ kind: "outside_window", current: candidate, previous }); continue; }
  ...
  for (const [identity, previous] of before) {
    ...
    const outside = previous.dateConfidence !== "low" && previous.publishedAt != null && previous.publishedAt < current.run.rangeFrom;
    result.changes.push({ kind: outside ? "outside_window" : changedPlan || !comparable(previous.source) || previous.publishedAt == null || previous.dateConfidence === "low" || previous.publishedAt > current.run.rangeTo ? "unknown" : "not_found", previous });
  }
  return result;
}
