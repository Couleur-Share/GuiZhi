...
MCP server（设置 → 数据 → MCP 接入）：交接稿的下一步——不用先复制再粘，
...
- MCP 只有 FTS 检索，没有语义检索：query 向量要调 embedding 接口，而 AI 配置的
  ...
  沿用列表投影的 snippet，视频条目开头的元数据引用块（含最长 300 字的简介）会
  占掉片段的大部分——剥离它需要在压平后的文本里猜边界，猜错会吃掉正文开头，
  比多几十字冗余更糟，所以没做。配置里的路径是绝对路径，应用换安装位置后要回
  设置页重新复制一次；也没有发 npm 包，所以没法 `npx` 一行接入。
