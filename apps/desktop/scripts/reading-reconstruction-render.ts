...
async function main() {
  ...
  const data = JSON.parse(await fs.readFile(path.join(directory, "accepted.json"), "utf8"));
  for (const fixture of data.fixtures) {
    ...
    for (const mode of ["offline", "embedded"]) await fs.writeFile(fixture.files[mode], themedReadingDocument(fixture.page, mode === "embedded" ? "reconstruction-fixture" : undefined, urls));
  }
