...
/**
 * 编译指纹的一部分：改动它等于让全库页面重编一遍。
 *
 * 所以只有「换了会产生实质不同知识」的改动才配升版本。纯排版约束
 * （例如禁止正文重复页面标题）不升——那类问题由确定性清洗兜底
 * （`shared/utils/wiki-body.ts`），不值得让用户烧一整轮 AI 调用。
 */
...
export const WIKI_COMPILE_SYSTEM_PROMPT =
  ...
  "body 不要以页面标题作为开头的一级标题，直接从正文或二级小标题写起；\n" +
