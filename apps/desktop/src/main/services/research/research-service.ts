...
export class ResearchService {
  ...
  private read(candidate: Parameters<ResearchReader>[0], signal: AbortSignal): Promise<ResearchDocument> {
    const task = this.options.read!(candidate, signal); this.readerTasks.add(task);
  ...
  createAndRun(input: CreateResearchRunInput): ResearchRun {
    ...
    this.workflow.setContext(run.id, { seriesId: run.id, phase: "searching", policyVersion: RESEARCH_POLICY.version, reportOutdated: false, knowledgeScope: input.knowledgeScope });
  ...
  cloneAndRun(id: string, replan = false): ResearchRun {
    ...
    const now = Date.now();
    const run = this.store.create({ topic: previous.topic, dayRange: previous.dayRange, depth: previous.depth, sources: [...previous.sources], knowledgeScope: previous.context?.knowledgeScope }, now - previous.dayRange * DAY_MS, now);
    ...
    this.workflow.setContext(run.id, { seriesId: previous.context?.seriesId ?? id, phase: "searching", policyVersion: RESEARCH_POLICY.version, reportOutdated: false, knowledgeScope: previous.context?.knowledgeScope, plan: replan ? undefined : previous.context?.plan });
    this.track(this.run(run.id).catch((error) => this.finishUnexpected(run.id, error)));
  ...
  verifyAndRetrySource(
    id: string,
    source: Exclude<ResearchSource, "bilibili">,
  ...
  private async run(
    ...
    if (run.depth === "deep" && this.options.read) await this.readCandidates(id, controller.signal);
