...
const CONFIG_DIR_NAME = "GuiZhi";
const CONFIG_FILE_NAME = "data-path.json";
export const LEGACY_PRODUCT_NAME = "GuiZhi";
...
export function hasExistingAppData(targetPath: string): boolean {
  return inspectDataPath(targetPath).hasExistingData;
}
...
export function inspectDataPath(targetPath: string): DataPathInspection {
  ...
  const knowledgeDbStat =
    dataDirStat?.isDirectory() === true && !dataDirStat.isSymbolicLink()
      ? readLinkSafeStats(knowledgeDbPath)
      : null;
  if (knowledgeDbStat?.isFile() && !knowledgeDbStat.isSymbolicLink()) {
    markers.push({
      name: "data/knowledge.db",
...
export function resolveInitialUserDataPath(
  ...
  if (
    ...
    // the user's existing data in %APPDATA%/GuiZhi to become invisible.
